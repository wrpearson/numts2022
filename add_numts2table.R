...
sn.data <- read.table(file='all_mtGenome_accs_class_size.tab',header=TRUE,sep='\t')
...
if (NROW(no_data) > 0) {
    zero_data <- data.frame(taxon_id=no_data$NCBI_Tax_ID, numt_cnt=0, numt_len=0)
    message("zero_data)")
    message(zero_data)
...
summ_data_s <- summ_data_s[!duplicated(summ_data_s$taxon_id),c('taxon_id','numt_cnt','numt_len')]
...
## head(new.gt.data)
new_col_order = c('NCBI_Tax_ID','s_name','Species','Common_name','Mitochondrial_genome_accession','Nuclear_genome_accession','Nuclear_genome_size_.GB.','RefSeq.GenBank','dataset','numt_cnt','numt_len')
