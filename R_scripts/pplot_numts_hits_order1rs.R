...
p.name<-'pplot_numts_hits_order1rs.R'
...
hc_meds2$display = sprintf("%.0f/%.0f",hc_meds2$h_cnt.refseq,hc_meds2$h_cnt.genbank)
## hc_meds2
...
if (Sys.getenv(c("PLOT_DOLAB"))=='none') {
    pdf(file=paste0(pdf_dir, t_name, "_ord1rs_p_NL.pdf"),width=10, height=7)
    pA / pB
} else {
    pdf(file=paste0(pdf_dir, t_name, "_ord1rs_p.pdf"),width=10, height=7.3)
