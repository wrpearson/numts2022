...
## pplot_numts_alen_mat0x.R -- 0 versions of plot programs assume no self-matches
##

################
## pplot_numts_alen_matx.R -- plot box plots of all data by matrix and coverage -- .tab files in data/, results in figs/
## allows an exclusion argument to ignore some taxon_ids
##
################
##
...
p.name<-'pplot_numts_hits_alen_mat0x.R'
...
exclude_file = Sys.getenv(c("EXCLUDE_TAXA"))
exclude_df=NULL
if ( exclude_file != '') {
    exclude_df <- read.table(file=Sys.getenv(c("EXCLUDE_TAXA")),sep='\t',header=TRUE,stringsAsFactors=TRUE)
}
...
print(NROW(exclude_df))
if (NROW(exclude_df)>0) {
    print(sprintf("excluding %s: %d rows; current: %d",exclude_file,NROW(exclude_df),NROW(summ_data)))
    summ_data <- summ_data[! summ_data$taxon_id %in% exclude_df$taxon_id,]
    print(sprintf("excluded: %d",NROW(summ_data)))
}
...
pd <- position_jitterdodge(jitter.width=0.1,jitter.height=0.1, dodge.width=0.75, seed=12345)
...
pA <- ggplot(summ_data,aes(x=matrix,y=h_cnt, shape=matrix, fill=g_type)) +
  ...
  geom_point(aes(group=interaction(s_name, g_type), shape=matrix, color=g_type),size=2,alpha=0.7, position=pd) +
  ...
  scale_x_discrete(name="search strategy and scoring matrix")

if (Sys.getenv(c("JOIN_PTS"))=='join') {
    print("joining points with lines")
    pA <- pA + geom_line(aes(group=interaction(s_name,g_type), color=g_type),alpha=0.35, position=pd)
}

pA <- pA + facet_wrap(~alen_f,labeller=labeller(alen_f=alen_fs),nrow=2) # +ggtitle(paste("A. by matrix type",plot.title))


if (Sys.getenv(c("PLOT_DOLAB"))=='none') {
    pdf(file=paste0(pdf_dir, t_name, "_alen_mat0x_NL.pdf"),width=10.0, height=6.0)
    pA
} else {
    pdf(file=paste0(pdf_dir, t_name, "_alen_mat0x.pdf"),width=10.0, height=6.0)
