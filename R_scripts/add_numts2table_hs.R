...
for (file in hit_files) {
    ...
    ## BLN higher stringency
    hit_data_hs = hit_data[hit_data$expect < ifelse(hit_data$src=='d2',1e-10,1e-6),]
    ## same stringency
    ## hit_data_hs = hit_data[hit_data$expect < 1e-10,]

    ## cat("hit_data_hs\n")
    ## print(NROW(hit_data_hs))

    all_tfx = NROW(hit_data[hit_data$src != 'd2',])
    all_bg  = NROW(hit_data[hit_data$src == 'd2',])

    hs_tfx  = NROW(hit_data_hs[hit_data_hs$src != 'd2',])
    hs_bg   = NROW(hit_data_hs[hit_data_hs$src == 'd2',])

    print(sprintf("all_tfx: %d; hs_tfx: %d; fn: %.3f",all_tfx, hs_tfx, hs_tfx/all_tfx))
    print(sprintf("all_bg: %d; hs_bg: %d; fn: %.3f",all_bg, hs_bg, hs_bg/all_bg))
    ...
    t_data_hs <- aggregate(cbind(numt_cnt_hs=qlen) ~ taxon_id, data=hit_data_hs, FUN=NROW)

    ## cat("t_data_hs\n")
    ## print(head(t_data_hs))

    t_data = merge(t_data, t_data_hs[,c('taxon_id','numt_cnt_hs')],by='taxon_id')

    t_data$hs_fract = round(ifelse(t_data$numt_cnt>0,t_data$numt_cnt_hs/t_data$numt_cnt,0),4)*100.0
...
if (NROW(no_data) > 0) {
    zero_data <- data.frame(taxon_id=no_data$NCBI_Tax_ID, numt_cnt=0, numt_cnt_hs=0, hs_fract=0, numt_len=0, med_len=0, max_len=0)
    message("zero_data")
    message(zero_data)
...
summ_data_s <- summ_data_s[!duplicated(summ_data_s$taxon_id),c('taxon_id','numt_cnt','numt_len','med_len','max_len','hs_fract')]
...
## head(new.gt.data)
new_col_order = c('NCBI_Tax_ID','s_name','Species','Common_name','Mitochondrial_genome_accession','Nuclear_genome_accession','Nuclear_genome_size_.GB.','RefSeq.GenBank','dataset','numt_cnt','numt_len','med_len','max_len','hs_fract')

new.gt.data <- new.gt.data[order(new.gt.data$s_name),]

cat("fivenum hs_fract\n")
fivenum(new.gt.data[new.gt.data$hs_fract > 0,]$hs_fract)
fivenum(new.gt.data[new.gt.data$hs_fract > 10.0,]$hs_fract)
